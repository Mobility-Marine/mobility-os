    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "grid", gap: "14px", alignContent: "start" }}>
          ...
          {/* DATOS BÁSICOS */}
          <Field label={`${tl.providerName ?? "Nombre del proveedor"} *`}>
            <input value={form.name ?? ""} onChange={(e) => set("name", e.target.value)} placeholder="Nombre de la empresa" style={INPUT} />
          </Field>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
            <Field label={tl.rfc ?? "RFC"}>
              <input value={form.rfc ?? ""} onChange={(e) => set("rfc", e.target.value.toUpperCase())} placeholder="RFC123456ABC" style={INPUT} />
            </Field>
            <Field label={tl.taxId ?? "Tax ID / EIN"}>
              <input value={form.tax_id ?? ""} onChange={(e) => set("tax_id", e.target.value)} placeholder="Para proveedores USA" style={INPUT} />
            </Field>
            <Field label={tl.scacCode ?? "Código SCAC"}>
              <input value={form.scac_code ?? ""} onChange={(e) => set("scac_code", e.target.value.toUpperCase())} placeholder="ABCD (carriers USA)" style={INPUT} />
            </Field>
            <Field label={tl.paymentTerms ?? "Condiciones de pago"}>
              <input value={form.payment_terms ?? ""} onChange={(e) => set("payment_terms", e.target.value)} placeholder="15 días / Crédito 30" style={INPUT} />
            </Field>
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--color-border-faint)", paddingTop: "12px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
              <Field label={tl.contactName ?? "Nombre"}>
                <input value={form.contact_name ?? ""} onChange={(e) => set("contact_name", e.target.value)} style={INPUT} />
              </Field>
              <Field label={tl.contactPhone ?? "Teléfono"}>
                <input value={form.contact_phone ?? ""} onChange={(e) => set("contact_phone", e.target.value)} style={INPUT} />
              </Field>
              <div style={{ gridColumn: "1 / -1" }}>
                <Field label={tl.contactEmail ?? "Email"}>
                  <input type="email" value={form.contact_email ?? ""} onChange={(e) => set("contact_email", e.target.value)} style={INPUT} />
                </Field>
              </div>
            </div>
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--color-border-faint)", paddingTop: "12px" }}>
            <Field label={tl.coverageRoutes ?? "Rutas / Cobertura"}>
              <input value={form.coverage_routes ?? ""} onChange={(e) => set("coverage_routes", e.target.value)} placeholder="GDL-CDMX, México-USA, etc." style={INPUT} />
            </Field>
          </div>
          <Field label={tl.servicesOffered ?? "Servicios que ofrece"}>
            <textarea rows={2} value={form.services_offered ?? ""} onChange={(e) => set("services_offered", e.target.value)} placeholder="FTL, LTL, despacho aduanal…" style={{ ...INPUT, height: "auto", padding: "8px 12px", resize: "vertical" }} />
          </Field>
          <Field label={tl.notes ?? "Notas"}>
            <input value={form.notes ?? ""} onChange={(e) => set("notes", e.target.value)} placeholder="Observaciones internas…" style={INPUT} />
          </Field>
