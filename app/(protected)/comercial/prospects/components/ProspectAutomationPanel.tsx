  ...
  <div style={{
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", gap: "6px" }}>
        {criticals > 0 && (
          <span style={{ padding: "2px 8px", borderRadius: "var(--radius-full)", background: "var(--color-danger-bg)", border: "1px solid var(--color-danger-border)", fontSize: "11px", fontWeight: 600, color: "var(--color-danger-text)" }}>
            {criticals} {t.dashboard.criticalItems.toLowerCase()}
          </span>
        )}
        {highs > 0 && (
          <span style={{ padding: "2px 8px", borderRadius: "var(--radius-full)", background: "var(--color-warning-bg)", border: "1px solid var(--color-warning-border)", fontSize: "11px", fontWeight: 600, color: "var(--color-warning-text)" }}>
    ...
    {alerts.length === 0 ? (
      ...
      <div style={{ display: "grid", gap: "8px", overflowY: "auto", flex: 1 }}>
        {alerts.slice(0, 8).map((alert) => {
          ...
          return (
            <div key={alert.id} onClick={() => onSelect(prospect)} style={{
            ...
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.opacity = "1"; }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
                ...
                <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 6px", borderRadius: "var(--radius-full)", background: sev.color + "20", color: sev.color }}>
                  {alert.severity}
                </span>
              </div>
              <div style={{ fontSize: "12px", color: "var(--color-text-second)", lineHeight: 1.5 }}>{desc}</div>
              <div style={{ fontSize: "11px", fontWeight: 600, color: "var(--color-text-muted)", display: "flex", alignItems: "center", gap: "4px" }}>
                {alert.prospectName}
