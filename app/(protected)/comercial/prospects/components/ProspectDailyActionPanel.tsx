...
export default function ProspectDailyActionPanel({ prospects, onSelect }: Props) {
  ...
  <div style={{
    ...
    {totalActions === 0 ? (
      ...
      <div style={{ display: "grid", gap: "10px", overflowY: "auto", flex: 1 }}>
        {sections.map((sec) => {
          ...
          return (
            <div key={sec.key} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 6px", borderRadius: "var(--radius-full)", background: sec.color + "20", color: sec.color }}>
                  {sec.items.length}
                </span>
              </div>
              <div style={{ display: "grid", gap: "4px" }}>
                {sec.items.slice(0, 5).map((p) => (
                  <div key={p.id} onClick={() => onSelect(p)} style={{
                    ...
                    background: "var(--color-bg-base)", border: `1px solid ${sec.border}`,
