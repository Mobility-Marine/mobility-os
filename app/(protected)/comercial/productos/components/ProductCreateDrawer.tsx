      ...
      <div style={{
        ...
        <div style={{ padding: "18px 24px", borderBottom: "1px solid var(--color-border-faint)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
            <div>
              <div style={{ fontSize: "16px", fontWeight: 800, color: "var(--color-text-primary)" }}>Nuevo producto</div>
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "10px" }}>
                ...
                <Field label="Nombre" required>
                  <input value={form.name ?? ""} onChange={(e) => set("name", e.target.value)} placeholder="Nombre del producto" style={INPUT} />
                </Field>
              </div>
              <Field label="Descripción / Especificaciones">
                <textarea
                  ...
                  placeholder="Medidas, calibre, material, especificaciones técnicas…"
                  style={{ ...INPUT, height: "auto", padding: "8px 12px", resize: "vertical" }}
                />
              </Field>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <Field label="Categoría">
                  <input value={form.category ?? ""} onChange={(e) => set("category", e.target.value)} placeholder="Embalaje, Refacciones…" style={INPUT} />
                </Field>
              ...
              <Field label="Notas internas">
                <input value={form.notes ?? ""} onChange={(e) => set("notes", e.target.value)} placeholder="Proveedor habitual, observaciones…" style={INPUT} />
              </Field>
            </>
          )}
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "11px", color: "var(--color-text-muted)", marginTop: "2px" }}>
                      Ganancia: ${(form.unit_price - (form.cost ?? 0)).toLocaleString("es-MX", { minimumFractionDigits: 2 })} / unidad
                    </div>
                  </div>
          ...
          {step === "fiscal" && (
            <>
              <div style={{ padding: "10px 14px", borderRadius: "var(--radius-md)", background: "var(--color-info-bg)", border: "1px solid var(--color-info-border)", fontSize: "12px", color: "var(--color-info-text)", lineHeight: 1.6 }}>
                Los datos fiscales son opcionales al crear el producto, pero <strong>obligatorios para facturar</strong>. Puedes completarlos después desde el workspace del producto.
              </div>
              <div style={{ display: "grid", gap: "10px" }}>
                <Field label="Clave de producto SAT">
                ...
                <Field label="Descripción de la fracción">
        ...
        <div style={{ padding: "14px 24px", borderTop: "1px solid var(--color-border-faint)", display: "flex", gap: "10px", flexShrink: 0 }}>
          ...
          {step !== "fiscal" ? (
            ...
            <button onClick={handleCreate} disabled={saving} style={{
              ...
              {saving ? t.general.loading : "Crear producto"}
            </button>
          )}
