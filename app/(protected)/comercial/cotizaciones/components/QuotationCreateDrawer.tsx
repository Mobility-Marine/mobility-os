...
export default function QuotationCreateDrawer({ open, onClose, onCreate }: Props) {
  ...
  function canAdvance(): boolean {
    ...
    if (step === "items")  return quotType === "products" ? items.length > 0 : services.length > 0;
    return true;
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await onCreate(
        ...
        quotType === "services" ? services : undefined,
      );
      handleClose();
              ...
              {services.length > 0 && (
                <div style={{ display: "grid", gap: "6px" }}>
                  {services.map((svc, i) => {
                    ...
                    const label = (t.quot as any)?.[cfg.labelKey.replace("quot.", "")] ?? svc.service_type;
                    return (
                      <div key={i} style={{ padding: "10px 12px", borderRadius: "var(--radius-md)", background: "var(--color-bg-subtle)", border: "1px solid var(--color-border-faint)", display: "flex", gap: "10px", alignItems: "center" }}>
                        <span style={{ fontSize: "10px", fontWeight: 700, padding: "2px 6px", borderRadius: "var(--radius-full)", background: cfg.color + "20", color: cfg.color, flexShrink: 0 }}>{label}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--color-text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{svc.description}</div>
                          {svc.origin && <div style={{ fontSize: "10px", color: "var(--color-text-muted)" }}>{svc.origin} → {svc.destination}</div>}
                        </div>
                        <span style={{ fontSize: "13px", fontWeight: 800, color: "var(--color-success-text)", fontVariantNumeric: "tabular-nums" }}>
                          {svc.currency} ${Number(svc.price).toLocaleString("es-MX", { minimumFractionDigits: 2 })}
                        </span>
                        <button onClick={() => setServices((p) => p.filter((_, idx) => idx !== i))} style={{ width: "22px", height: "22px", borderRadius: "var(--radius-sm)", background: "var(--color-danger-bg)", border: "1px solid var(--color-danger-border)", cursor: "pointer", color: "var(--color-danger-text)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                        </button>
                      </div>
                    );
          ...
          {step === "preview" && (
            <>
              ...
              <div style={{ background: "var(--color-bg-subtle)", border: "1px solid var(--color-border-faint)", borderRadius: "var(--radius-md)", padding: "12px" }}>
                ...
                {quotType === "services" && services.map((svc, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "4px" }}>
                    <span style={{ color: "var(--color-text-second)" }}>{svc.service_type} — {svc.description.substring(0, 30)}</span>
                    <span style={{ color: "var(--color-text-primary)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
                      {svc.currency} ${Number(svc.price).toLocaleString("es-MX", { minimumFractionDigits: 2 })}
                    </span>
                  </div>
