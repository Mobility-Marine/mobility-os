...
const EMPTY_LINE = (transport: TipoTransporte): LineDraft => ({
  description: "", quantity: "",
  unit_label:  UNITS_BY_TRANSPORT[transport][0],
...
export default function ContentOpCompleta({ info, setInfo, billingConcepts, setBillingConcepts, svcCatalog }: Props) {
  ...
  const [lineForm,      setLineForm]      = useState<LineDraft>(EMPTY_LINE("fcl"));
                          ...
                          <div style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ fontSize: "10px", color: "var(--color-text-muted)", marginTop: "2px" }}>
                                {qty} {uLabel} × {cur} ${Number(uPrice).toLocaleString("es-MX", { minimumFractionDigits: 2 })} · {taxLabel}{(line as any).notes && ` · ${(line as any).notes}`}
                              </div>
                            </div>
                    ...
                    <div style={{ background: "var(--color-bg-base)", border: `1px dashed ${editingLine !== null ? "var(--color-warning-border)" : "var(--color-border)"}`, borderRadius: "var(--radius-md)", padding: "12px", display: "flex", flexDirection: "column", gap: "10px" }}>
                      ...
                      <Field label="Descripción *">
                        <input value={lineForm.description} onChange={e => setLineForm(p => ({ ...p, description: e.target.value }))}
                          placeholder="Flete, honorarios, impuestos, maniobras…" style={INPUT} />
                      </Field>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr 1fr 1fr", gap: "8px" }}>
                        <Field label="Cantidad *">
                          <input type="number" value={lineForm.quantity} onChange={e => setLineForm(p => ({ ...p, quantity: e.target.value }))} placeholder="1" style={INPUT} />
                        </Field>
                        <Field label="Unidad de cobro *">
                          <select value={lineForm.unit_label} onChange={e => setLineForm(p => ({ ...p, unit_label: e.target.value }))} style={SELECT}>
                            {UNITS_BY_TRANSPORT[info.tipo_transporte].map(u => <option key={u} value={u}>{u}</option>)}
                          </select>
                        </Field>
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        ...
                        {editingLine !== null && (
                          <button onClick={() => { setLineForm(EMPTY_LINE(info.tipo_transporte)); setEditingLine(null); }}
            ...
            <div style={{ padding: "14px", borderRadius: "var(--radius-md)", background: "var(--color-bg-subtle)", border: "2px solid var(--color-brand-blue)", display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <Field label="Nombre del concepto (visible en PDF) *">
                <input value={conceptForm.description} onChange={e => setConceptForm(p => ({ ...p, description: e.target.value }))}
                  placeholder="ej: Operación Completa Importación FCL Manzanillo" style={INPUT} />
              </Field>
