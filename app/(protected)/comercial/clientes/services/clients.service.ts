...
export async function createClient(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      billing_ext_number:   payload.billing_ext_number    ?? null,
      billing_int_number:   payload.billing_int_number    ?? null,
      billing_neighborhood: payload.billing_neighborhood  ?? null,
      billing_city:         payload.billing_city          ?? null,
      billing_state:        payload.billing_state         ?? null,
      billing_country:      payload.billing_country       ?? "México",
    })
    .select("*")
    .single();
