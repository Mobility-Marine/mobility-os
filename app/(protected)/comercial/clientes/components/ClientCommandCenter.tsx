...
export default function ClientCommandCenter({ clients, onSelect }: Props) {
  ...
  const withBalance = active.filter((c) => (c.stats?.openBalance ?? 0) > 0);
  ...
  const cards = [
    ...
    },
    {
      key:    "balance",
      label:  "Con saldo",
      hint:   withBalance.length > 0 ? `${withBalance.length} con saldo pendiente` : "Cobranza al día",
      value:  withBalance.length,
      color:  withBalance.length > 0 ? "var(--color-warning-text)" : "var(--color-success-text)",
      bg:     withBalance.length > 0 ? "var(--color-warning-bg)"   : "var(--color-success-bg)",
      border: withBalance.length > 0 ? "var(--color-warning-border)": "var(--color-success-border)",
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
