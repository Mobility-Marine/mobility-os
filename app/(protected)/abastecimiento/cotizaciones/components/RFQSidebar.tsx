...
export default function RFQSidebar({ rfqs, selected, onSelect, filters, setFilters, onNew }: Props) {
  const { t, lang } = useTranslation();
  const tp          = (t.procurement as any) ?? {};
  const locale      = lang === "en" ? "en-US" : "es-MX";

  function getStatusLabel(s: RFQStatus) {
    ...
    <div style={{ background: "var(--color-bg-base)", border: "1px solid var(--color-border-faint)", borderRadius: "var(--radius-lg)", padding: "14px", display: "flex", flexDirection: "column", gap: "10px", height: "100%", minHeight: 0, overflow: "hidden" }}>
      <div style={{ flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
          <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--color-text-muted)", textTransform: "uppercase", letterSpacing: "1px" }}>{tp.rfqs ?? "Cotizaciones"}</span>
          <span style={{ fontSize: "11px", fontWeight: 700, padding: "1px 7px", borderRadius: "var(--radius-full)", background: "var(--color-bg-subtle)", border: "1px solid var(--color-border-faint)", color: "var(--color-text-muted)" }}>{rfqs.length}</span>
        </div>
        ...
        <div style={{ position: "relative", marginBottom: "8px" }}>
          ...
          <input placeholder={tp.searchRfq ?? "Buscar…"} value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} style={{ width: "100%", height: "32px", paddingLeft: "28px", paddingRight: "8px", borderRadius: "var(--radius-md)", border: "1px solid var(--color-border)", background: "var(--color-bg-subtle)", color: "var(--color-text-primary)", fontSize: "12px", outline: "none", boxSizing: "border-box" }} />
        </div>

        <div style={{ display: "flex", gap: "3px" }}>
          {([
            { v: "all",               l: "Todas"  },
            { v: "draft",             l: tp.rfqDraft       ?? "Borrador" },
            { v: "responses_received",l: tp.rfqResponsesIn ?? "Resp."    },
            { v: "awarded",           l: tp.rfqAwarded     ?? "Adjud."   },
          ] as { v: RFQStatus | "all"; l: string }[]).map((f) => (
      ...
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0, display: "grid", gap: "5px", alignContent: "start" }}>
        {rfqs.length === 0 ? (
          <div style={{ padding: "28px 12px", textAlign: "center", color: "var(--color-text-muted)", fontSize: "13px" }}>{tp.noRfqs ?? "Sin solicitudes"}</div>
        ) : rfqs.map((r) => {
          ...
          return (
            <div key={r.id} onClick={() => onSelect(r)} style={{ padding: "10px 12px", borderRadius: "var(--radius-md)", background: isSelected ? "var(--color-bg-active)" : "var(--color-bg-subtle)", border: isSelected ? "1px solid var(--color-brand-blue)" : "1px solid var(--color-border-faint)", cursor: "pointer", display: "grid", gap: "3px", transition: "var(--transition-fast)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--color-text-muted)", fontFamily: "monospace" }}>{r.rfq_number ?? "—"}</span>
                <div style={{ flex: 1 }} />
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", color: "var(--color-text-muted)" }}>
                <span>{respCount} prov. cotizando</span>
                <span>{r.deadline ? new Date(r.deadline).toLocaleDateString(locale, { day: "numeric", month: "short" }) : ""}</span>
              </div>
            </div>
          );
        })}
