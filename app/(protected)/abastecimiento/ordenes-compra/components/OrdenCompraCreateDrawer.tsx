...
export default function OrdenCompraCreateDrawer({ open, suppliers, saving, onClose, onCreate }: Props) {
  ...
  // Seleccionar producto del catálogo → autocompletar form del ítem
            ...
            <>
              ...
              <div style={{ background: "var(--color-bg-subtle)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", padding: "14px", display: "grid", gap: "10px" }}>
                    ...
                    {filteredProducts.length === 0 ? (
                      ...
                      filteredProducts.slice(0, 20).map((p) => (
                        <div
                          ...
                          style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", cursor: "pointer", borderBottom: "1px solid var(--color-border-faint)", transition: "background 0.1s" }}
                ...
                <div style={{ fontSize: "10px", color: "var(--color-text-muted)", marginTop: "-4px" }}>
                  {es
                    ? "Selecciona un producto para autocompletar — o llena los campos manualmente."
                    : "Select a product to autofill — or fill fields manually."}
                </div>
              </div>
              ...
              <div style={{ background: "var(--color-bg-subtle)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", padding: "14px", display: "grid", gap: "10px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "3fr 1fr", gap: "10px" }}>
                  <div>
                    ...
                    <input value={itemForm.description} onChange={(e) => setIF("description", e.target.value)} placeholder={es ? "Nombre o descripción del producto…" : "Product name or description…"} style={INPUT} />
                  </div>
                  <div>
                    <div style={{ fontSize: "10px", fontWeight: 600, color: "var(--color-text-muted)", marginBottom: "4px" }}>
                      {es ? "Unidad" : "Unit"}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", gap: "8px", alignItems: "end" }}>
                  ...
                  <button
                    onClick={addItem}
                    disabled={!itemForm.description.trim() || !itemForm.unit_price}
                    style={{ height: "32px", padding: "0 14px", borderRadius: "var(--radius-md)", background: itemForm.description.trim() && itemForm.unit_price ? "var(--color-brand-blue)" : "var(--color-bg-base)", color: itemForm.description.trim() && itemForm.unit_price ? "#fff" : "var(--color-text-muted)", border: "1px solid var(--color-border)", fontSize: "12px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}
                  >
                    + {es ? "Agregar" : "Add"}
                  </button>
                </div>
              ...
              {items.length > 0 && (
                <div style={{ display: "grid", gap: "5px" }}>
                  {items.map((item, i) => {
                    ...
                    const linkedProduct = item.product_id ? products.find((p) => p.id === item.product_id) : null;
                    return (
                      <div key={i} style={{ display: "flex", gap: "8px", padding: "8px 12px", borderRadius: "var(--radius-md)", background: "var(--color-bg-subtle)", border: "1px solid var(--color-border-faint)", alignItems: "center" }}>
                        ...
                        <div style={{ fontSize: "12px", fontWeight: 700, color: "var(--color-success-text)", fontVariantNumeric: "tabular-nums", flexShrink: 0 }}>
                          ${fmt(base)}
                        </div>
                        <button onClick={() => setItems((p) => p.filter((_, idx) => idx !== i))} style={{ width: "22px", height: "22px", borderRadius: "var(--radius-sm)", background: "var(--color-danger-bg)", border: "1px solid var(--color-danger-border)", cursor: "pointer", color: "var(--color-danger-text)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
