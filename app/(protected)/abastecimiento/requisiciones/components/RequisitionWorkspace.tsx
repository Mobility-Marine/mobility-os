...
export default function RequisitionWorkspace({
  requisition, saving, onUpdate, onStatusChange, onDelete, onUpsertItem, onDeleteItem,
}: Props) {
  ...
  async function handleSave() {
    await onUpdate(requisition.id, form);
  ...
  async function handleSaveItem() {
    ...
    setAddingItem(false);
    setItemDesc(""); setItemQty("1"); setItemUnit("pza"); setItemPrice(""); setItemNotes("");
  }
      ...
      <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--color-border-faint)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          {!editing && nextOpts.map((opt) => (
            <button key={opt.next} onClick={() => {
              if (opt.next === "rejected") { setRejectModal(true); return; }
              onStatusChange(requisition.id, opt.next);
            }} style={{ height: "28px", padding: "0 10px", borderRadius: "var(--radius-md)", background: opt.bg, border: `1px solid ${opt.border}`, color: opt.color, fontSize: "11px", fontWeight: 700, cursor: "pointer" }}>
              {opt.label} →
            </button>
          ))}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px" }}>
        <div style={{ display: "grid", gap: "18px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              <Field label={tp.requisitionTitle ?? "Título"}>
                {editing
                  ? <input value={(form as any).title ?? ""} onChange={(e) => set("title", e.target.value)} style={INPUT} />
                  : <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--color-text-primary)" }}>{requisition.title}</div>
                }
              </Field>
            </div>
            ...
            <Field label={tp.neededBy ?? "Fecha requerida"}>
              {editing
                ? <input type="date" value={(form as any).needed_by ?? ""} onChange={(e) => set("needed_by", e.target.value)} style={INPUT} />
                : <div style={{ fontSize: "12px", color: "var(--color-text-primary)" }}>{requisition.needed_by ? new Date(requisition.needed_by).toLocaleDateString(locale) : <span style={{ color: "var(--color-text-muted)" }}>—</span>}</div>
              }
            </Field>

            <Field label={tp.department ?? "Área"}>
              {editing
                ? <input value={(form as any).department ?? ""} onChange={(e) => set("department", e.target.value)} style={INPUT} />
                : <div style={{ fontSize: "12px", color: "var(--color-text-primary)" }}>{requisition.department ?? <span style={{ color: "var(--color-text-muted)" }}>—</span>}</div>
              }
            </Field>
          ...
          <div>
            ...
            {addingItem && (
              <div style={{ padding: "12px 14px", borderRadius: "var(--radius-md)", background: "var(--color-bg-subtle)", border: "1px solid var(--color-border)", display: "grid", gridTemplateColumns: "1fr 80px 80px 100px 80px", gap: "8px", alignItems: "end", marginBottom: "8px" }}>
                <div>
                ...
                <div>
                  <div style={{ fontSize: "9px", fontWeight: 600, color: "var(--color-text-muted)", marginBottom: "3px", textTransform: "uppercase" }}>Precio est.</div>
                ...
                <div style={{ display: "flex", gap: "4px" }}>
                  <button onClick={handleSaveItem} disabled={!itemDesc.trim()} style={{ flex: 1, height: "30px", borderRadius: "var(--radius-sm)", background: "var(--color-brand-blue)", color: "#fff", border: "none", fontSize: "11px", fontWeight: 700, cursor: "pointer" }}>✓</button>
                  <button onClick={() => { setAddingItem(false); setItemDesc(""); setItemQty("1"); setItemUnit("pza"); setItemPrice(""); }} style={{ width: "30px", height: "30px", borderRadius: "var(--radius-sm)", background: "var(--color-bg-base)", border: "1px solid var(--color-border)", color: "var(--color-text-muted)", fontSize: "11px", cursor: "pointer" }}>✕</button>
                </div>
              </div>
            )}
            ...
            {items.length === 0 ? (
              <div style={{ padding: "24px", textAlign: "center", color: "var(--color-text-muted)", fontSize: "13px", border: "1px dashed var(--color-border)", borderRadius: "var(--radius-md)" }}>
                Sin ítems. Agrega lo que necesitas comprar.
              </div>
            ) : (
              <div style={{ borderRadius: "var(--radius-md)", border: "1px solid var(--color-border-faint)", overflow: "hidden" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 70px 70px 100px 24px", gap: "8px", padding: "7px 12px", background: "var(--color-bg-subtle)", borderBottom: "1px solid var(--color-border-faint)" }}>
                  {["Descripción","Cant.","Unidad","Precio est.",""].map((h) => (
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", padding: "8px 12px", background: "var(--color-bg-subtle)", borderTop: "2px solid var(--color-border-faint)", gap: "12px" }}>
                  <span style={{ fontSize: "11px", fontWeight: 600, color: "var(--color-text-muted)", textTransform: "uppercase" }}>Total estimado</span>
        ...
        <>
          ...
          <div style={{ position: "fixed", left: "50%", top: "50%", transform: "translate(-50%,-50%)", background: "var(--color-bg-base)", borderRadius: "var(--radius-lg)", padding: "24px", width: "min(440px, 90vw)", boxShadow: "var(--shadow-xl)", zIndex: 501, display: "grid", gap: "14px" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={() => setRejectModal(false)} style={{ flex: 1, height: "38px", borderRadius: "var(--radius-md)", background: "var(--color-bg-subtle)", border: "1px solid var(--color-border)", color: "var(--color-text-second)", fontSize: "13px", cursor: "pointer" }}>
                {t.general.cancel}
              </button>
