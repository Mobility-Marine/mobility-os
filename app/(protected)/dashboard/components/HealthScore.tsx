...
export default function HealthScore({ metrics }: HealthScoreProps) {
  ...
  const factors = [
    { label: "Comercial",  value: metrics.activeProspects > 0 ? "Activo" : "Sin actividad", ok: metrics.activeProspects > 0 },
    { label: "Logística",  value: metrics.activeShipments > 0 ? `${metrics.activeShipments} embarques` : "Sin embarques", ok: true },
    { label: "Finanzas",   value: metrics.pendingInvoices === 0 ? "Al día" : `${metrics.pendingInvoices} pendientes`, ok: metrics.pendingInvoices === 0 },
    { label: "Actividad",  value: metrics.openQuotations > 0 ? `${metrics.openQuotations} cotizaciones` : "Sin cotizaciones", ok: metrics.openQuotations > 0 },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: "18px", fontWeight: 700, color: labelColor, marginBottom: "4px" }}>
            {label}
          </div>
          <div style={{ fontSize: "12px", color: "var(--color-text-muted)", lineHeight: 1.5 }}>
            {score >= 80
              ? "La empresa opera en condiciones óptimas."
              : score >= 60
              ? "Operación estable con áreas de mejora."
              : "Se requiere atención en módulos clave."}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gap: "6px" }}>
        {factors.map((f) => (
          <div key={f.label} style={{
            ...
            <span style={{
              fontSize: "11px",
              color: f.ok ? "var(--color-success-text)" : "var(--color-danger-text)",
              fontWeight: 600,
            }}>
              {f.value}
            </span>
            ...
            return (
              <div key={i} style={{ flex: 1, height: "100%", display: "flex", alignItems: "flex-end" }}>
                <div style={{
                  ...
                  background: isLast ? labelColor : "var(--color-border)",
                  opacity: isLast ? 1 : 0.5 + (i / scoreHistory.length) * 0.5,
