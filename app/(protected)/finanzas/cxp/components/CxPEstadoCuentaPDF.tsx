...
import { AP_SUPPLIER_TYPE_CONFIG } from "../types/cxp.types";
...
const STATUS_LABELS: Record<string, string> = { pending: "Pendiente", partial: "Parcial", paid: "Pagado", disputed: "En disputa", cancelled: "Cancelado" };
...
const AGING_COLORS: Record<string, string> = { "0-30": "#16a34a", "31-60": "#d97706", "61-90": "#f97316", "+90": "#dc2626" };
const TYPE_LABELS: Record<string, string> = { procurement: "Abastecimiento", logistics: "Logística", operating: "Operativo" };

export default function CxPEstadoCuentaPDF({ supplier, records, payments, settings, generatedAt }: Props) {
  ...
  const WHITE = "#ffffff"; const LIGHT = "#f8fafc";
  const TEXT_DARK = "#1a2332"; const TEXT_MEDIUM = "#334155"; const TEXT_MUTED = "#64748b";
  ...
  const issuerLocation = issuerState && issuerCountry ? `${issuerState}, ${issuerCountry}` : issuerState || issuerCountry || issuerAddress;
  const footerText     = issuerName + (issuerLocation ? ` · ${issuerLocation}` : "") + (issuerRfc ? ` · RFC: ${issuerRfc}` : "") + (issuerPhone ? ` · ${issuerPhone}` : "");
    ...
    <>
      <View style={s.header}>
        <View style={{ flexDirection: "column", gap: 3 }}>
          {logoUrl ? <Image src={logoUrl} style={s.logoBox} /> : <Text style={{ fontSize: 20, fontWeight: "bold", color: HEADER_TEXT }}>{issuerName}</Text>}
        ...
        <View style={{ alignItems: "flex-end", gap: 4 }}>
          <Text style={{ fontSize: 8, color: ACCENT, textTransform: "uppercase", letterSpacing: 2 }}>Estado de Cuenta Proveedor</Text>
          <Text style={{ fontSize: 16, fontWeight: "bold", color: HEADER_TEXT, letterSpacing: 1 }}>{supplier.name}</Text>
          {supplier.rfc ? <Text style={{ fontSize: 8, color: HEADER_TEXT_SUB, fontFamily: "Courier" }}>{supplier.rfc}</Text> : null}
          <View style={{ backgroundColor: BRAND_COLOR, borderRadius: 4, padding: "3 8" }}>
            <Text style={{ color: BRAND_TEXT, fontSize: 8, fontWeight: "bold" }}>{TYPE_LABELS[supplier.type] ?? supplier.type}</Text>
          </View>
    ...
    <Document>
      <Page size="LETTER" style={s.page}>
        ...
        <View style={s.body}>
          ...
          {/* Info proveedor + período */}
          <View style={[s.section, s.row2]}>
            <View style={s.col}>
              <Text style={s.sectionTitle}>Información del proveedor</Text>
              <Text style={s.value}>{supplier.name}</Text>
              {supplier.rfc ? <Text style={[s.muted, { marginTop: 3 }]}>{`RFC: ${supplier.rfc}`}</Text> : null}
              <Text style={[s.muted, { marginTop: 3 }]}>{TYPE_LABELS[supplier.type] ?? supplier.type}</Text>
            </View>
            <View style={s.col}>
          ...
          <View style={s.summaryBox}>
            {[
              ...
              { l: "Total pagado",     v: `${currency} $${fmt(totalPagado)}`,    color: "#16a34a"     },
              { l: "Saldo pendiente",  v: `${currency} $${fmt(totalPendiente)}`, color: totalPendiente > 0 ? "#dc2626" : "#16a34a" },
            ].map(c => (
              <View key={c.l} style={[s.summaryCard, { borderLeftColor: c.color }]}>
          ...
          <View style={s.section}>
            ...
            <View style={s.tableHeader}>
              ...
              <Text style={[s.tableHeaderTxt, { flex: 0.8, textAlign: "center" }]}>Vence</Text>
              <Text style={[s.tableHeaderTxt, { flex: 0.9, textAlign: "center" }]}>Estado</Text>
            </View>
            {records.map((ap, i) => {
              const aging      = ap.aging_bucket ?? "0-30";
              const agingColor = AGING_COLORS[aging] ?? "#64748b";
              const scfg       = STATUS_COLORS[ap.status] ?? { color: "#64748b", bg: "#f1f5f9" };
              return (
                <View key={ap.id} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                  <Text style={[s.cellTxt, { flex: 1.2, fontFamily: "Courier" }]}>{ap.document_number || "—"}</Text>
                  <Text style={[s.cellMuted, { flex: 1 }]}>{new Date(ap.document_date).toLocaleDateString("es-MX")}</Text>
                  <Text style={[s.cellMuted, { flex: 1 }]}>{ap.due_date ? new Date(ap.due_date).toLocaleDateString("es-MX") : "—"}</Text>
                  <Text style={[s.cellTxt, { flex: 1, textAlign: "right", fontWeight: "bold" }]}>${fmt(ap.total)}</Text>
                  <Text style={[s.cellTxt, { flex: 1, textAlign: "right", color: "#16a34a" }]}>${fmt(ap.paid_amount)}</Text>
                  <Text style={[s.cellTxt, { flex: 1, textAlign: "right", fontWeight: "bold", color: ap.balance > 0 ? "#dc2626" : "#16a34a" }]}>${fmt(ap.balance)}</Text>
                  <View style={{ flex: 0.8, alignItems: "center" }}>
                    <View style={[s.badge, { backgroundColor: `${agingColor}20` }]}>
                      <Text style={[s.badgeTxt, { color: agingColor }]}>{`${ap.days_due ?? 0}d`}</Text>
                    </View>
                  </View>
                  <View style={{ flex: 0.9, alignItems: "center" }}>
                    <View style={[s.badge, { backgroundColor: scfg.bg }]}>
                      <Text style={[s.badgeTxt, { color: scfg.color }]}>{STATUS_LABELS[ap.status] ?? ap.status}</Text>
                    </View>
                  </View>
                </View>
              );
          ...
          {payments.length > 0 && (
            <View style={s.section}>
              <Text style={s.sectionTitle}>Historial de pagos realizados</Text>
              <View style={s.tableHeader}>
                <Text style={[s.tableHeaderTxt, { flex: 1   }]}>Fecha</Text>
          ...
          <View style={s.totalBox}>
            {[
              ...
              { l: "Total pagado",    v: `${currency} $${fmt(totalPagado)}`    },
            ].map(r => (
            ...
            <View style={[s.totalRow, { borderTopWidth: 1, borderTopColor: BORDER_COLOR, paddingTop: 7, marginTop: 4 }]}>
              <Text style={s.grandLabel}>SALDO POR PAGAR</Text>
