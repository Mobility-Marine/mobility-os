...
function NowIndicator({ weekDays }: { weekDays: Date[] }) {
  ...
  return (
    <div style={{ position: "absolute", top: `${top}px`, left: `calc(56px + ${todayIdx} * ${colWidth})`, width: colWidth, height: "2px", background: "var(--color-brand-orange)", zIndex: 3, pointerEvents: "none" }}>
      <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "var(--color-brand-orange)", marginTop: "-3px", marginLeft: "-4px" }} />
    </div>
  );
}

export default function WeekView({ weekDays, events, selectedDate, onEventClick, onSlotClick, onEventDrop }: WeekViewProps) {
  ...
  return (
    <div style={{ background: "var(--color-bg-base)", border: "1px solid var(--color-border-faint)", borderRadius: "var(--radius-lg)", overflow: "hidden", boxShadow: "var(--shadow-sm)" }}>
      ...
      {/* HEADER DÍAS */}
      <div style={{ display: "grid", gridTemplateColumns: "56px repeat(7, minmax(0, 1fr))", borderBottom: "1px solid var(--color-border-faint)", background: "var(--color-bg-subtle)" }}>
        <div />
        {weekDays.map((day, i) => {
          const isToday    = getLocalDateISO(day) === todayStr;
          const isSelected = getLocalDateISO(day) === selectedDate;
          return (
            <div key={i} style={{ padding: "10px 8px", textAlign: "center", borderLeft: "1px solid var(--color-border-faint)" }}>
              ...
              <div style={{
                ...
                margin: "4px auto 0", fontSize: "14px", fontWeight: 700,
              }}>
                {day.getDate()}
              </div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "56px repeat(7, minmax(0, 1fr))" }}>
          {HOURS.map((hour) => (
            <React.Fragment key={hour}>
              <div style={{ padding: "0 8px", height: `${HOUR_HEIGHT}px`, display: "flex", alignItems: "flex-start", paddingTop: "4px", fontSize: "10px", fontWeight: 600, color: "var(--color-text-muted)", borderTop: "1px solid var(--color-border-faint)", flexShrink: 0 }}>
                {hour}
              </div>
              {weekDays.map((day, di) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    const d = new Date(day);
                    d.setHours(Number(h), 0, 0, 0);
                    onSlotClick(`${getLocalDateISO(d)}T${h.padStart(2, "0")}:00`);
                  }}
                  style={{ height: `${HOUR_HEIGHT}px`, borderTop: "1px solid var(--color-border-faint)", borderLeft: "1px solid var(--color-border-faint)", cursor: "pointer", transition: "background var(--transition-fast)" }}
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "4px", overflow: "hidden" }}>
                  ...
                  {isModule && (
                    <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "rgba(255,255,255,0.8)", flexShrink: 0 }} />
                  )}
                </div>
                {height > 40 && (
